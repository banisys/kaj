...
@section('content')
    <div class="container mt-5 pb-5" id="area" style="position: relative">
        <span style="position: absolute;margin-right:10px;top: -14px;background-color: #f4f6f9;color: #9f9f9f;">
            افزودن دسته برای ویژگی ها
        </span>
        <div class="row" style="border: 1px #dedede solid;padding:35px 5px 35px 5px;border-radius: 10px">
            <div class="col-md-12">
                <div class="form-inline">
                    <label for="exampleFormControlSelect1" class="ml-3">دسته بندی :</label>
        ...
        <div class="row mt-4">
            <div class="col-md-12">
                <table class="table table-striped" v-if="catspecs">
                    <thead>
                    <tr>
                        <th scope="col">نام</th>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" name="title" class="form-control" v-model="search.cat" @keyup="searchCat"
                                   placeholder="جستجو بر اساس دسته">
                        </td>
                        <td></td>
                    </tr>
                    <tr v-for="(catspec,index) in catspecs.data">
                        <td>@{{catspec.name}}</td>
                        <td>@{{ catspec.cat.name }}</td>
                        <td>
                            <a @click="deleteCatspec(catspec.id)" style="font-size: 20px;">
        ...
        <div class="row mt-3" v-if="catspecs">
            <pagination :limit="1" :data="catspecs" @pagination-change-page="fetchCatspecs"
    ...
    <script>
        var app = new Vue({
            ...
            methods: {
                ...
                fetchCatspecs(page = 1) {
                    let _this = this;
                    axios.get('/admin/catspec/fetch?page=' + page).then(res => {
                        _this.catspecs = res.data;
                    });
                },
                deleteCatspec(id) {
                    swal.fire({
                        ...
                        if (result.value) {
                            axios.get(`/admin/catspec/delete/${id}`)
                                .then((res) => {
                                    if (res.data === "cant") {
                                        swal.fire(
                                            {
                ...
                searchId(page = 1) {
                    ...
                    if (this.search.id.length > 0) {
                        axios.get('/admin/catspec/search?page=' + page, {params: {id: this.search.id}}).then(response => {
                            data.catspecs = response.data;
                        });
                    }
                    if (this.search.id.length === 0) {
                        this.fetchCatspecs();
                ...
                searchName(page = 1) {
                    ...
                    if (this.search.name.length > 0) {
                        axios.get('/admin/catspec/search?page=' + page, {params: {name: this.search.name}}).then(response => {
                            data.catspecs = response.data;
                        });
                    }
                    if (this.search.name.length === 0) {
                ...
                searchCat(page = 1) {
                    ...
                    if (this.search.cat.length > 0) {
                        axios.get('/admin/catspec/search?page=' + page, {params: {cat: this.search.cat}}).then(response => {
                            data.catspecs = response.data;
                ...
                formSubmit(e) {
                    e.preventDefault();
                    let _this = this;
                    axios.post('/admin/catspec/store', {
                        name: this.form.name,
                        cat: this.form.cat,
                        ...
                        if (Array.isArray(x.name)) {
                            _this.error.name = this.allerros.name[0];
                        }

                        if (Array.isArray(x.cat)) {
                            _this.error.cat = this.allerros.cat[0];
                        }
                    });
                },
            },
